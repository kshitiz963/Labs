...
if let returnValue = checkAge("20") {
    print(returnValue)
}
...
func makePurchase(_ item: String) -> Double? {
    for (key, value) in stock{
        if key == item && value > 0 {
            if let price = prices[key] {
                return price
            }
            stock[key] = value - 1      // update like this
        }
    }
    return nil
}

if let price = makePurchase("Chips") {
    print("Your purchase total is: \(price)")
} else {
    print("That item is currently out of stock.")
}

if let price = makePurchase("Donuts") {
    print("Your purchase total is: \(price)")
} else {
    print("That item is currently out of stock.")
}
